...
const getAll = async (req, res) => {
    try {
        const result = await mongodb.getDb().db('tienda_organica').collection('carnes_lacteos').find();
...
const getSingle = async (req, res) => {
    try {
        ...
        const result = await mongodb.getDb().db('tienda_organica').collection('carnes_lacteos').find({ _id: productId });
        const lists = await result.toArray();
...
const createProduct = async (req, res) => {
    try {
        ...
        const response = await mongodb.getDb().db('tienda_organica').collection('carnes_lacteos').insertOne(product);
...
const updateProduct = async (req, res) => {
    try {
        ...
        const response = await mongodb.getDb().db('tienda_organica').collection('carnes_lacteos').replaceOne({ _id: productId }, product);
...
const deleteProduct = async (req, res) => {
    try {
        ...
        const productId = new ObjectId(req.params.id);
        const response = await mongodb.getDb().db('tienda_organica').collection('carnes_lacteos').deleteOne({ _id: productId });
